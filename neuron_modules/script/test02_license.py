...
def move_and_delete_logs():
    ...
    if os.path.exists("build/logs/neuron.log"):
        shutil.copy2("build/logs/neuron.log",
                     "neu-ft/neuron_modules/report/test02_license_neuron.log")
        os.remove("build/logs/neuron.log")
    if os.path.exists("build/logs/modbus-tcp-node.log"):
        shutil.copy2("build/logs/modbus-tcp-node.log",
                     "neu-ft/neuron_modules/report/test02_license_modbus_tcp_node.log")
...
class TestLicense:
    ...
    def add_tags_more_than_30(self, random_port):
        ...
        response = read(test_data=tag_read_data, header_data=config.headers)
        assert 200 == response.status_code
        assert NEU_ERR_LICENSE_INVALID == response.json()[
            'tags'][0].get("error")
    ...
    def test02_upload_invalid_license_fail(self, setup_and_teardown_neuron):
        ...
        response = upload_license(
            test_data=license_data_invalid, header_data=config.headers)
        assert 400 == response.status_code
        assert NEU_ERR_LICENSE_INVALID == response.json().get("error")

    # def test03_tag_less_than_30_without_license_success(self, setup_and_teardown_neuron, setup_and_teardown_modbus, random_port):
        # print("---given:without license, when:add 2 tags and write&read, then:failed---")
