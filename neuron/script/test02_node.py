...
def setup_and_teardown_neuron():
    ...
    assert err == b'', "stderr not empty: " + err.decode()
...
def move_and_delete_logs():
    ...
    if os.path.exists("./logs/neuron.log"):
        shutil.copy2("./logs/neuron.log", f"{start_dir}/{report_directory}/test02_node_neuron.log")
        os.remove("./logs/neuron.log")
    os.chdir(start_dir)

class TestNode:

    with open('data/test02_node_data.json') as f:
